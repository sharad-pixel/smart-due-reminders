...
export const getPersonaTone = (name: string): PersonaToneConfig | null => {
  const key = name.toLowerCase();
  return personaTones[key] || null;
};

// Get persona by days past due
export const getPersonaToneByDaysPastDue = (daysPastDue: number): PersonaToneConfig | null => {
  if (daysPastDue === 0) return null;
  
  const personas = Object.values(personaTones);
...
export const getPersonaToneByBucket = (bucket: string): PersonaToneConfig | null => {
  switch (bucket) {
    case 'current':
      return null; // No persona for current invoices
    case 'dpd_1_30':
...
// Tone intensity modifiers for the gauge
