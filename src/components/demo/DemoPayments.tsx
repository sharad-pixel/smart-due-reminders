  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center">
        <motion.p key={recoveredAmount} initial={{ scale: 1.3 }} animate={{ scale: 1 }}
      ...
      <div className="max-w-lg mx-auto space-y-3">
        {paidInvoices.map((inv, i) => (
          <motion.div key={inv.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
      ...
      {done && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center pt-4">
          <Button size="lg" onClick={nextStep}>
            Next: Export Data <ArrowRight className="ml-2 h-4 w-4" />
          </Button>
        </motion.div>
      )}
    </div>
