...
serve(async (req) => {
  ...
  try {
    ...
    if (isServiceCall) {
      ...
      console.log("[DAILY-INTELLIGENCE] Service/cron call detected — processing all users");
      ...
      if (usersError) {
        console.error("[DAILY-INTELLIGENCE] Error fetching user IDs:", usersError);
      ...
      targetUserIds = [...new Set((activeUserIds || []).map((d: any) => d.user_id))];
      console.log(`[DAILY-INTELLIGENCE] Found ${targetUserIds.length} unique users with active accounts`);
    } else {
      // User-initiated call: validate JWT and process only their accounts
      ...
      targetUserIds = [effectiveAccountId || user.id];
      console.log(`[DAILY-INTELLIGENCE] User call for account: ${targetUserIds[0]}`);
    }
    ...
    for (const accountId of targetUserIds) {
      ...
      if (debtorsError) {
        console.error(`[DAILY-INTELLIGENCE] Error fetching debtors for ${accountId}:`, debtorsError);
        totalFailed++;
        continue;
      }

      console.log(`[DAILY-INTELLIGENCE] User ${accountId}: ${debtors?.length || 0} accounts`);
...
- collectionStrategy: recommended approach for this account`;
          ...
          if (!aiResponse.ok) {
            ...
            totalFailed++;
            allErrors.push(`${debtor.company_name || debtor.name}: AI request failed`);
            continue;
          }
          ...
          if (updateError) {
            ...
            totalFailed++;
            allErrors.push(`${debtor.company_name || debtor.name}: Failed to save report`);
          } else {
            totalProcessed++;
          }
          ...
          totalFailed++;
          allErrors.push(`${debtor.company_name || debtor.name}: ${err instanceof Error ? err.message : 'Unknown error'}`);
        }
      }
    }

    console.log(`[DAILY-INTELLIGENCE] Completed. Users: ${targetUserIds.length}, Processed: ${totalProcessed}, Failed: ${totalFailed}`);
