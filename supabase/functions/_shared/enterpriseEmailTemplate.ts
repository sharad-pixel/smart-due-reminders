 ...
 * 
 * Used for: Daily Digest, Welcome Email, Password Reset,
 * Email Verification, Team Invitations, Account notifications
 */
...
// Legacy brain SVGs kept for compatibility
...
/**
 * Recouply.ai wordmark
 */
function wordmark(size: number = 22, white: boolean = false): string {
  if (white) {
    return `<span style="font-size: ${size}px; font-weight: 700; letter-spacing: -0.3px; font-family: ${FONT_STACK}; color: #ffffff;">Recouply<span style="color: ${BRAND.accent};">.ai</span></span>`;
  }
  return `<span style="font-size: ${size}px; font-weight: 700; letter-spacing: -0.3px; font-family: ${FONT_STACK}; color: ${BRAND.foreground};">Recouply<span style="color: ${BRAND.accent};">.ai</span></span>`;
}
...
export function enterpriseFooter(): string {
  ...
  return `
    <div style="background-color: #1e293b; padding: 24px; text-align: center;">
      ...
      <p style="color: #475569; margin: 0; font-size: 10px; font-family: ${FONT_STACK};">
        © ${year} RecouplyAI Inc. · Delaware, USA
      </p>
    </div>`;
}
