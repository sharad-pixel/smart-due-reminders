...
serve(async (req) => {
  ...
  try {
    ...
    if (!webhookSecret) {
      logStep("STRIPE_WEBHOOK_SECRET not configured - rejecting webhook");
      ...
      case 'invoice.payment_succeeded': {
        ...
        if (subscriptionId) {
          ...
          if (updateError) {
            ...
            logStep("Subscription updated after successful payment", { customerId, planType, billingInterval });
          }
        } else {
          ...
          logStep("One-time payment cleared failure state", { customerId });
        }
        break;
      }
