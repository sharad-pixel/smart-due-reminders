...
import { BrandingConfig, renderEmail, getSenderIdentity } from "../_shared/renderBrandedEmail.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: branding } = await supabaseClient
      ...
      .select("logo_url, business_name, from_name, email_signature, email_footer, primary_color, accent_color, ar_page_public_token, ar_page_enabled, include_portal_link_in_outreach, stripe_payment_link, email_format, email_wrapper_enabled, sending_mode, from_email, from_email_verified, verified_from_email, reply_to_email, footer_disclaimer")
