...
BEGIN
  SELECT i.user_id, i.id INTO v_invoice_user_id, v_invoice_id
  FROM invoices i
  ...
  SELECT jsonb_build_object(
    'invoice', jsonb_build_object(
      ...
      'processing_fee_percent', i.processing_fee_percent,
      'processing_fee_amount', i.processing_fee_amount,
      'due_date', i.due_date,
    ...
    'line_items', (
      ...
      ) ORDER BY li.sort_order ASC, li.created_at ASC), '[]'::jsonb)
    ...
    'payments', (
      ...
      ) ORDER BY pay.payment_date ASC), '[]'::jsonb)
      FROM payments pay
      WHERE pay.invoice_id = v_invoice_id
        AND pay.reconciliation_status != 'rejected'
    ),
    'transactions', (
      SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      ) ORDER BY tx.transaction_date ASC), '[]'::jsonb)
      FROM invoice_transactions tx
      WHERE tx.invoice_id = v_invoice_id
        AND tx.transaction_type IN ('credit', 'write_off', 'payment')
    )
  ) INTO v_result
  FROM invoices i
