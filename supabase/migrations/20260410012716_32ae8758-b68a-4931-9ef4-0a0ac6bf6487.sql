...
BEGIN
  ...
  -- Resolve effective account (check if user belongs to an org)
  ...
  -- Use effective account or fall back to invoice user_id
  v_effective_account_id := COALESCE(v_effective_account_id, v_invoice_user_id);

  -- Check if the account owner has public links enabled
